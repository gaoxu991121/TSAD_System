...
from Preprocess.Window import convertToSlidngWindow
...
class LSTMEncoder(nn.Module):
    ...
    def forward(self,x):
        ...
        lstm_output,(cell,hidden) = self.lstm(x)
        cell = cell.permute(1,0,2)

        z_mean = self.z_mean_layer(cell)
        z_log_var = self.z_log_var_layer(cell)
...
class LSTMDecoder(nn.Module):
    ...
    def forward(self,z):
        ...
        output1,(cell,hidden) = self.lstm(z)
        cell = cell.permute(1, 0, 2)
        x_mean = self.x_mean_layer(cell)
        x_log_var = self.x_log_var_layer(cell)
...
class LSTMVAE(nn.Module):
    ...
    def processData(self,data_train,data_test):
        ...
        data_train = convertToSlidngWindow(data = data_train,window_size = window_size)
        data_test = convertToSlidngWindow(data = data_test,window_size = window_size)
